...
public class ActivePhaseServer(BattlePhaseMachine.Context _context) : IBattlePhase
{
    ...
    public void Enter()
    {
        _subscriptions =
        [
            Bus.Global.Subscribe<AttackClickedEvent>(_ => _step.Change(new SelectingAttackTargetStepServer(_context))),
            Bus.Global.Subscribe<MagicClickedEvent>(_ => _step.Change(new SelectingSpellStepServer(_context))),
            Bus.Global.Subscribe<ActivePhaseBackClickedEvent>(BackClicked),
            Bus.Global.Subscribe<SpellSelectedEvent>(_ => _step.Change(new SelectingMagicTargetStepServer(_context))),
            Bus.Global.Subscribe<IdleStepServer.CompletedEvent>(evnt => Bus.Global.Publish(new CompletedEvent(evnt.CurrentUnit)))
        ];
